...
pick_branch <- function (state, env)
{
  ...
  if (length(ls(env))) {
    ...
    if (length(m) > 1) {
      abort(msg("global environment matched more than once against history, will not attach"))
    }
  ...
  if (length(lv) == 1) {
    ...
    lv <- nth(lv, last(order(map_int(lv, `[[`, i = 'time'))))
  }

  inform(msg("attaching to repository, commit ", lv$id))
